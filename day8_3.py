...
#                       WHERE 절;
#                         ''')
# id가 1인 레코드에서 책제목(title)의 데이터값을 '맥스'로 수정하여라
...
# cursor.execute(sql변수, (값1, 조건절 값2))

# id가 1인 레코드에서 가격(price)의 데이터값을 50000으로 수정하여라
# sql2 = '''UPDATE book1
#              SET price = ?
#            WHERE id = ?; '''
...
# ----- 레코드 삭제2
# sql변수 = DELETE FROM 테이블명 WHERE 절에서 데이터값은 물음표? ;
